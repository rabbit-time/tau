...
class System(commands.Cog):
    ...
    async def tag(self, ctx):
        '''Create a tag.
        Tags are generated by referencing an existing message which will copy the message's content.
        A tag cannot have the same name as an existing command.
        Deleting the original message can cause images not to load properly.\n
        **Example:```yml\n♤tag make rules 546397670793805825\n♤tag delete rules\n♤tag alias```**
        '''
        pass

    @tag.command(name='make', usage='make <name> <message>')
    @commands.has_guild_permissions(manage_guild=True)
    @commands.bot_has_permissions(external_emojis=True)
    @guild_only()
    async def make(self, ctx, name: str, msg: discord.Message):
        # make sure name isn't already a command or an alias
        reserved = {cmd.name for cmd in self.bot.commands}
        for cmd in self.bot.commands:
            reserved.update(cmd.aliases)
        
        if name in reserved:
            raise commands.BadArgument

        # get embed from message and convert JSON string
        em = json.dumps(msg.embeds[0].to_dict()) if msg.embeds else '{}'

        # save
        await self.bot.tags.update((ctx.guild.id, name), 'embed', em)
        await self.bot.tags.update((ctx.guild.id, name), 'content', msg.content)

        file = File('assets/dot.png', 'unknown.png')
        embed = Embed(description=f'You can now reference this tag using **`{ctx.prefix}{name}`**', color=utils.Color.sky)
        embed.set_author(name='Tag created', icon_url='attachment://unknown.png')

        await ctx.reply(embed=embed, file=file, mention_author=False)
    
    @tag.command(name='delete', aliases=['del'], usage='delete <name>')
    @commands.has_guild_permissions(manage_guild=True)
    @commands.bot_has_permissions(external_emojis=True)
    @guild_only()
    async def delete(self, ctx, name: str):
        if self.bot.tags.get((ctx.guild.id, name)):
            # if exists, delete
            await self.bot.tags.delete((ctx.guild.id, name))
